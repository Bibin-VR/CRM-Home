  ...
  const columns = [
    ...
    { key: "inventory.quantityReserved", label: "Reserved", render: (_v: any, row: any) => row.inventory?.quantityReserved || 0 },
    {
      ...
      render: (_v: any, row: any) => {
        ...
        const isLow = qty < 20;
        return (
          <span className={`font-mono-data ${isLow ? "text-[#F43F5E] font-bold" : "text-[#2DD4BF]"}`}>
    ...
    <DashboardLayout>
      <div className="mb-4 flex items-center gap-4">
        <button
          ...
          className={`brutalist-btn text-xs flex items-center gap-2 ${showLowStock ? "bg-[#F43F5E] text-white border-[#F43F5E]" : ""}`}
        >
